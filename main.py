...
def load_vgg(sess, vgg_path):
    ...
    w1=gragh.get_tensor_by_name(vgg_input_tensor_name)
    keep=gragh.get_tensor_by_name(vgg_keep_prob_tensor_name)
    layer3=gragh.get_tensor_by_name(vgg_layer3_out_tensor_name)
    layer4=gragh.get_tensor_by_name(vgg_layer4_out_tensor_name)
    layer7=gragh.get_tensor_by_name(vgg_layer7_out_tensor_name)
...
def run():
    def process_image(image):
        return helper.process_image_detail(sess, logits, keep_prob, input_image, image_shape,image)
    num_classes = 2
    ...
    with tf.Session() as sess:
        ...
        train_nn(sess,100,5,get_batches_fn,train_op,cross_entropy_loss,input_image,correct_label,keep_prob,learning_rate)
        # TODO: Save inference data using helper.save_inference_samples
        helper.save_inference_samples(runs_dir, data_dir, sess, image_shape, logits, keep_prob, input_image)

        # OPTIONAL: Apply the trained model to a video
        # white_output = 'project_out_video.mp4'
